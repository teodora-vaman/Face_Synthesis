...
class Encoder(nn.Module):
    def __init__(self, img_channels = 3, attribute_number = 1, dim_zgomot = 256):
        ...
        self.encoder_qPhi = nn.Sequential(
            nn.Conv2d(in_channels=img_channels, out_channels=32, kernel_size=3, stride=2, padding=1, bias=False),
            nn.ReLU(True),
            nn.Conv2d(in_channels=32, out_channels=64, kernel_size=5, stride=1, padding=2, bias=False), # nr_img x 32 x 64 x 64
            nn.ReLU(True),
            nn.BatchNorm2d(64),
